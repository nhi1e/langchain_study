...
db_path = os.path.join(os.path.dirname(__file__), "mock_data", "mock_support.db")

# Create SQLite connection manually (if needed)
conn = sqlite3.connect(db_path)

# Create LangChain-compatible SQLDatabase object
url = URL.create("sqlite", database=db_path)
db = SQLDatabase.from_uri(url, include_tables=["support_tickets"])
...
state = {"user_id": "u005", "question": "What was the issue with the most recent support ticket?"}
...
async def query_support_ticket(user_id: str, question: str):
    """
    Handles SQL queries for support tickets and returns an answer string.
    """
    state = {"user_id": user_id, "question": question}
    result = graph.invoke(state)
    return result["answer"] if "answer" in result else "No answer found."
